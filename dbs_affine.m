...
%
function [new_img, new_M] = dbs_affine(old_img, old_M, new_elem_size, verbose, bg, method)

   if ~exist('old_img','var') | ~exist('old_M','var')
      error('Usage: [new_img new_M] = dbs_affine(old_img, old_M, [new_elem_size], [verbose], [bg], [method]);');
   end
   ...
   dbs_dispercent(0,'Reslicing volumes');

   for z = 1:new_dim(3)

      if verbose && ~mod(z,10)
      	 dbs_dispercent(z/new_dim(3));
      end
