function Y=dbs_detect_edges_3d(varargin)
...
dbs_dispercent(0,'Sampling');

for slice=1:dimensionality
    if useimgtbx
    edgedslice=edge(squeeze(X(:,:,slice)),'canny',alpha); 
    else
    edgedslice=dbs_edgedetection(squeeze(X(:,:,slice)),alpha);
    end
    Y(:,:,slice)=edgedslice;
    dbs_dispercent(slice/dimensionality);
end
dbs_dispercent(100,'end');


function outimg=dbs_edgedetection(inimg,alpha)
