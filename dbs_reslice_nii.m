...
%
function dbs_reslice_nii(old_fn, new_fn, voxel_size, verbose, bg, method, img_idx, preferredForm)
   ...
   if nii.hdr.dime.dim(5) > 1
      for i = 1:nii.hdr.dime.dim(5)
         [img(:,:,:,i) M] = dbs_affine(nii.img(:,:,:,i), old_M, voxel_size, verbose, bg, method);
      end
   else
      [img M] = dbs_affine(nii.img, old_M, voxel_size, verbose, bg, method);
   end
